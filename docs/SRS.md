...
JARA adalah aplikasi web untuk mengelola tugas pribadi maupun tim. Pengguna dapat membuat beberapa daftar tugas yang berfungsi sebagai list atau project, menentukan prioritas dan tenggat waktu, menandai tugas selesai, serta melihat progres. Pemilik daftar dapat menambahkan pengguna lain agar pekerjaan dapat dilakukan bersama. Sejak revisi 2026-09-16, satu tugas dapat diberikan kepada lebih dari satu peserta daftar (SRS-027). Administrator bertanggung jawab menambah, mengubah, dan menonaktifkan akun, serta dapat menghapus akun pengguna sesuai strategi penghapusan yang masih merupakan open decision (SRS-028, ADR-011).
...
| **SRS-028** | Penghapusan akun oleh administrator. **Pemilik: Muhammad Zaidaan Ardiyansyah.** *Supersedes bagian penonaktifan akun pada SRS-021 jika stakeholder memutuskan penghapusan menggantikan status `DISABLED`.* | - Hanya administrator aktif yang dapat melakukan penghapusan.<br>- Pengguna biasa menerima respons `403`.<br>- Administrator tidak dapat menghapus akunnya sendiri.<br>- Penghapusan tidak meninggalkan foreign key atau data relasi yang rusak.<br>- Seluruh perubahan relasi diproses secara atomic.<br>- Respons API mengikuti envelope standar.<br>- Strategi hard delete, soft delete, atau disable diputuskan dan dicatat dalam [DECISIONS.md](DECISIONS.md).<br>- Sebelum keputusan dibuat, strategi penghapusan ditandai sebagai open decision (ADR-011). |
...
1. Penghapusan akun pengguna oleh administrator kini berada dalam scope melalui SRS-028, dengan strategi (hard delete, soft delete, atau disable) yang masih merupakan open decision di ADR-011. Penghapusan akun tidak lagi otomatis dilarang; yang tetap wajib adalah tidak menyisakan foreign key atau data relasi yang rusak.
...
Fondasi, autentikasi, skema database, policy, seed data, dan quality tooling sudah tersedia. Endpoint list, membership, task, progress, dan admin-user sudah diimplementasikan beserta UI-nya pada arsitektur frontend saat ini. Endpoint berikut berstatus **planned** dan belum tersedia:

- `DELETE /api/admin/users/{user}` (penghapusan akun oleh administrator — SRS-028, menunggu keputusan ADR-011).
- Input `assignee_ids` multiple dan respons `assignees` pada endpoint tugas (SRS-027); API tugas saat ini masih single `assignee_id`.
- Integrasi shadcn preset `bhOibP160` (SRS-031) dan landing page publik `/` (SRS-032).
